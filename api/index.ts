...
app.get('/api/traffic/stream', async (req, res) => {
  ...
  const snapshot = () => {
    ...
    metrics.p50Ms = percentile(0.5); metrics.p90Ms = percentile(0.9); metrics.p95Ms = percentile(0.95); metrics.p99Ms = percentile(0.99);
    return { ...metrics };
  };
  ...
  const runWorker = async (workerId: number) => {
    while (!aborted) {
      ...
      if (enableMultiPage && subPaths.length) {
        ...
        requestUrl = selected.startsWith('http://') || selected.startsWith('https://') ? new URL(selected) : new URL(selected.startsWith('/') ? selected : `/${selected}`, target);
      }

      const started = performance.now();
      let statusCode = 0; let bytesReceived = 0; let success = false; let error = '';
      try {
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), timeoutMs);
        try {
          const response = await fetch(requestUrl, { signal: controller.signal, redirect: 'follow', headers: { 'User-Agent': 'ClickHeadLoadTest/1.0', Accept: 'text/html,application/xhtml+xml,application/json;q=0.9,*/*;q=0.8' } });
          statusCode = response.status;
          bytesReceived = (await response.arrayBuffer()).byteLength;
          success = response.ok;
      ...
      metrics.totalSent++; metrics.completed++; metrics.totalBytes += bytesReceived;
      if (success) metrics.successful++;
      if (statusCode >= 300 && statusCode < 400) metrics.redirects++;
      else if (statusCode >= 400 && statusCode < 500) metrics.clientErrors++;
      else if (statusCode >= 500) metrics.serverErrors++;
      else if (!success) metrics.networkErrors++;
      if (success) latencies.push(durationMs);

      send('metric', {
        metric: { id: requestId + 1, workerId, url: requestUrl.toString(), statusCode, durationMs, timestamp: Date.now(), success, error: error || undefined, userAgent: 'ClickHeadLoadTest/1.0', bytesReceived },
        aggregated: snapshot(),
  ...
  try {
    await Promise.all(Array.from({ length: Math.min(concurrency, totalRequests) }, (_, i) => runWorker(i + 1)));
    send('complete', { finalMetrics: snapshot(), message: aborted ? `Stopped after ${metrics.completed} requests.` : `Completed ${metrics.completed} requests.` });
  } finally {
