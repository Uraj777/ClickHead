...
async function validateTarget(raw: string): Promise<URL> {
  ...
  if (url.username || url.password) {
    throw new Error('Credential-bearing target URLs are not supported.');
  }
  ...
  if (!isDevelopment && addresses.some((entry) => isPrivateAddress(entry.address))) {
    throw new Error('Targets resolving to private or local networks are disabled in production.');
  }
...
app.get('/api/traffic/stream', async (req, res) => {
  ...
  let target: URL;
  try {
    target = await validateTarget(targetRaw);
  } catch (error) {
  ...
  req.on('close', () => { aborted = true; activeSessions.delete(sessionId); });
  ...
  const snapshot = () => {
    ...
    metrics.currentRps = metrics.completed / elapsedSeconds;
    metrics.avgRps = metrics.completed / elapsedSeconds;
    metrics.avgDurationMs = sorted.length ? Math.round(sorted.reduce((a, b) => a + b, 0) / sorted.length) : 0;
    metrics.minDurationMs = sorted[0] || 0;
    metrics.maxDurationMs = sorted[sorted.length - 1] || 0;
    metrics.p50Ms = percentile(0.5);
    metrics.p90Ms = percentile(0.9);
    metrics.p95Ms = percentile(0.95);
    metrics.p99Ms = percentile(0.99);
    return { ...metrics };
  };
  ...
  const runWorker = async (workerId: number) => {
    while (!aborted) {
      ...
      if (delayMs > 0 && requestId > 0) {
        const jitter = jitterMs > 0 ? Math.floor(Math.random() * (jitterMs + 1)) : 0;
        await new Promise((resolve) => setTimeout(resolve, delayMs + jitter));
      }
      if (aborted) return;

      let requestUrl = new URL(target.toString());
      if (enableMultiPage && subPaths.length) {
        const selected = subPaths[requestId % subPaths.length];
        if (selected.startsWith('http://') || selected.startsWith('https://')) requestUrl = new URL(selected);
        else requestUrl = new URL(selected.startsWith('/') ? selected : `/${selected}`, target);
      }
      ...
      try {
        ...
        try {
          const response = await fetch(requestUrl, {
            ...
            redirect: 'follow',
            headers: {
              'User-Agent': 'ClickHeadLoadTest/1.0',
      ...
      if (success) metrics.successful += 1;
      if (statusCode >= 300 && statusCode < 400) metrics.redirects += 1;
      else if (statusCode >= 400 && statusCode < 500) metrics.clientErrors += 1;
      else if (statusCode >= 500) metrics.serverErrors += 1;
      else if (!success) metrics.networkErrors += 1;
      if (success) latencies.push(durationMs);
      ...
      send('metric', {
        metric: {
          ...
          userAgent: 'ClickHeadLoadTest/1.0',
          bytesReceived,
        },
        aggregated: snapshot(),
        log: {
          ...
          message: success
            ? `[Worker ${String(workerId).padStart(2, '0')}] #${String(requestId + 1).padStart(4, '0')} HTTP ${statusCode} in ${durationMs}ms | ${requestUrl}`
            : `[Worker ${String(workerId).padStart(2, '0')}] #${String(requestId + 1).padStart(4, '0')} FAIL (${error || `HTTP ${statusCode}`}) in ${durationMs}ms | ${requestUrl}`,
        },
      });
    }
  };
  ...
  try {
    await Promise.all(Array.from({ length: Math.min(concurrency, totalRequests) }, (_, index) => runWorker(index + 1)));
    const finalMetrics = snapshot();
    send('complete', { finalMetrics, message: aborted ? `Stopped after ${metrics.completed} requests.` : `Completed ${metrics.completed} requests.` });
  } finally {
    activeSessions.delete(sessionId);
...
async function startServer() {
  ...
  app.listen(PORT, () => {
    console.log(`ClickHead server listening on http://localhost:${PORT}`);
  });
}
